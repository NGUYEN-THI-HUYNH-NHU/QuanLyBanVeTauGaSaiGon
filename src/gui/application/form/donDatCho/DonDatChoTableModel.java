...
public class DonDatChoTableModel extends AbstractTableModel {
	...
	private Consumer<DonDatCho> rowSelectionListener;
	...
	public DonDatCho getRow(int rowIndex) {
		if (rowIndex >= 0 && rowIndex < rows.size()) {
			return rows.get(rowIndex).getDonDatCho();
		}
		return null;
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		case COL_STT:
			return rowIndex + 1;
		case COL_DDC_ID:
			return row.getDonDatCho().getDonDatChoID();
		case COL_TEN_KH:
			return row.getDonDatCho().getKhachHang().getHoTen();
		case COL_CCCD_KH:
			return row.getDonDatCho().getKhachHang().getSoGiayTo();
		case COL_SDT_KH:
			return row.getDonDatCho().getKhachHang().getSoDienThoai();
		case COL_TONG_VE:
