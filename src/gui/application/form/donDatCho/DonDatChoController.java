...
public class DonDatChoController {
		...
		view.getTable().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (row >= 0 && column == DonDatChoTableModel.COL_XEM) {
					DonDatCho ddc = view.getTableModel().getRow(row);
		...
		if (!suggestions.isEmpty()) {
			for (String s : suggestions) {
				...
				if ("Mã đặt chỗ".equals(type)) {
					...
					item.setIcon(new FlatSVGIcon("gui/icon/svg/phone.svg", 0.6f));
				} else if ("Tên khách hàng".equals(type)) {
	...
	private void showModalChiTiet(DonDatCho ddc) {
		// Lấy danh sách vé chi tiết của đơn này
		List<Ve> listVe = veBUS.timCacVeTheoDonDatChoID(ddc.getDonDatChoID());
