...
public class PanelThongKeKhachHang extends JPanel {
	...
	public PanelThongKeKhachHang() {
		...
		cbPhanLoai = new JComboBox<>(
				new String[] { "Tất cả", "VIP", "Thân thiết", "Khách quay lại", "Khách mới", "Ngủ đông" });
		...
		// Tab 2: Bảng chi tiết
		String[] columnNames = { "STT", "Mã KH", "Tên Khách Hàng", "Khu Vực", "Loại Đối Tượng", "Số Lần Mua",
	...
	private void xuLyThongKe() {
		...
		try {
			if ("Theo ngày".equals(loaiThoiGian)) {
				...
				int m1 = cbTuThang.getSelectedIndex() + 1;
				int y1 = (Integer) cbTuNamThang.getSelectedItem();
				int m2 = cbDenThang.getSelectedIndex() + 1;
				int y2 = (Integer) cbDenNamThang.getSelectedItem();
				...
				int y1 = (Integer) cbTuNam.getSelectedItem();
				int y2 = (Integer) cbDenNam.getSelectedItem();
		...
		SwingWorker<ThongKeKHResult, Void> worker = new SwingWorker<>() {
			...
			protected ThongKeKHResult doInBackground() throws Exception {
				...
				res.chiTietKhachHang = thongKeKHDao.getThongKeKhachHang(finalFrom, finalTo, null, loaiDoiTuong,
						phanLoai);
	...
	private void capNhatChartVaTable(ThongKeKHResult res) {
		...
		if (res.countMoi > 0) {
			dataset.setValue("Mới (1 lần)", res.countMoi);
		}
		if (res.countQuayLai > 0) {
			dataset.setValue("Quay lại (2-4 lần)", res.countQuayLai);
		}
		if (res.countThanThiet > 0) {
			dataset.setValue("Thân thiết (>5 lần)", res.countThanThiet);
		}
		if (res.countVIP > 0) {
			dataset.setValue("VIP", res.countVIP);
		}
		if (res.countNguDong > 0) {
			dataset.setValue("Ngủ đông", res.countNguDong);
		}
		...
		if (res.chiTietKhachHang != null) {
			for (KhachHangRFM kh : res.chiTietKhachHang.values()) {
				chiTietTableModel.addRow(new Object[] { kh.khachHangID, kh.hoTen, kh.khuVuc, kh.loaiDoiTuong,
						kh.soLanMua, kh.tongChiTieu, kh.lanMuaCuoi != null ? kh.lanMuaCuoi.format(dateFormatter) : "",
						kh.phanLoai });
			}
		}
	...
	private void setupTableDetails() {
		...
		tableChiTiet.getColumnModel().getColumn(7).setCellRenderer(new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable t, Object v, boolean s, boolean f, int r, int c) {
				...
				} else if ("Khách quay lại".equals(val)) {
					comp.setForeground(new Color(41, 128, 185));
				} else if ("Ngủ đông".equals(val)) {
