...
public class PanelThongKe extends JPanel {
	...
	private JPanel createReportTabPanel() {
		...
		// Ghi chú (Đặt ở dòng 7, sử dụng thêm một panel nhỏ để JTextArea không bị kéo
		// căng)
...
// Listener cho nút Nhập tiền mặt
		btnNhapTienMat.addActionListener(e -> {
			...
			PanelBaoCao baoCaoPanel = new PanelBaoCao(nhanVien.getHoTen(), // tên NV
			...
			JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Lập Báo Cáo Giao Ca", true);
		...
		btnExport.addActionListener(ev -> {
			if (!isTienMatEntered) {
				JOptionPane.showMessageDialog(PanelThongKe.this,
						"Bạn phải nhập tiền mặt thực tế trước khi xuất báo cáo cuối ca.", "Yêu cầu",
		...
		SwingWorker<ThongKeResult, Void> worker = new SwingWorker<>() {
			...
			protected void done() {
				...
				try {
					...
					String tenNV = (nhanVien.getHoTen() != null && !nhanVien.getHoTen().isBlank()) ? nhanVien.getHoTen()
							: "Không xác định";
