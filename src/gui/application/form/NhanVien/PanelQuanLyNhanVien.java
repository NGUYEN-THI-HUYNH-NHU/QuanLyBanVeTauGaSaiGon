...
public class PanelQuanLyNhanVien extends JPanel implements ActionListener, MouseListener, KeyListener {
    ...
    private JLabel lblMaNVDetail, lblVaiTroDetail, lblTenNVDetail, lblGioiTinhDetail,
            ...
            lblNgayThamGiaDetail, lblTrangThaiDetail, lblCaLamDetail, lblAvatar;
    ...
    private JPanel panelChiTiet() {
        ...
        // Avatar
        lblAvatar = new JLabel(new ImageIcon(new ImageIcon(
                getClass().getResource("/gui/icon/png/avatar.png"))
    ...
    private boolean validForm() {
        ...
        if (!nhanVien_ctrl.validHoTen(ten)) {
            JOptionPane.showMessageDialog(this, "Tên không hợp lệ VD: Nguyễn Văn A", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
        ...
        if (!nhanVien_ctrl.validSDT(sdt)) {
            JOptionPane.showMessageDialog(this, "Số điện thoại không hợp lệ (đầu 0 theo dải nhà mạng VN.VD: 0912345678).", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
            txtSDT.requestFocus();
            return false;
        } else if (sdt.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Số điện thoại không được để trống", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
            txtSDT.requestFocus();
            return false;
        }
        ...
        if (!email.isEmpty() && !nhanVien_ctrl.validEmail(email)) {
            ...
            JOptionPane.showMessageDialog(this, "Email không được để trống", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
            txtEmail.requestFocus();
            return false;
        }
        // 4) Địa chỉ
        if (!nhanVien_ctrl.validDiaChi(diaChi)) {
            JOptionPane.showMessageDialog(this, "Địa chỉ không hợp lệ (tối thiểu 5 ký tự, cho phép chữ/số/khoảng trắng , . -). VD: 45/2 Nguyễn Huệ, Quận 1", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
        ...
        if (txtNgaySinh.getDate() == null) {
            ...
            LocalDate ns = txtNgaySinh.getDate().toInstant()
                    .atZone(java.time.ZoneId.systemDefault())
                    .toLocalDate();
            if (!nhanVien_ctrl.ngaySinh(ns)) {
                JOptionPane.showMessageDialog(this, "Ngày sinh không hợp lệ (phải trước ngày hiện tại).", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
                txtNgaySinh.requestFocus();
                return false;
            }
        }
        // 7) Ngày tham gia (nếu nhập phải đúng định dạng dd/MM/yyyy và không sau hôm nay)
        if (txtNgayThamGia.getDate() == null) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn ngày tham gia.", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
            txtNgayThamGia.requestFocus();
            return false;
        } else {
            LocalDate ntg = txtNgayThamGia.getDate().toInstant()
                    .atZone(java.time.ZoneId.systemDefault())
                    .toLocalDate();
            if (!nhanVien_ctrl.ngayThamGia(ntg)) {
                JOptionPane.showMessageDialog(this, "Ngày tham gia không hợp lệ (không được sau hôm nay).", "Lỗi dữ liệu", JOptionPane.WARNING_MESSAGE);
                txtNgayThamGia.requestFocus();
                return false;
            }
        }
    ...
    public void themNhanVien() {
        ...
        if (success) {
            ...
            JOptionPane.showMessageDialog(this, "Thêm nhân viên thất bại! Vui lòng kiểm tra lại.", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void suaNhanVien() {
        try {
            ...
            if (txtNgayThamGia.getDate() != null) {
                ngayThamGia = txtNgayThamGia.getDate().toInstant()
                        .atZone(java.time.ZoneId.systemDefault())
                        .toLocalDate();
            }
