...
/*
 * @(#) PanelSoDoChoDoiVe.java  1.0  [3:23:47 PM] Nov 20, 2025
 *
...
public class PanelSoDoChoDoiVe extends JPanel {
	...
	public void setController(DoiVeBuoc5Controller c) {
		this.controller = c;
	}
...
//	}
		...
		try {
			if (controller != null && controller.getSelectedChuyen() != null) {
				...
				if (chuyenID != null && gaDiID != null && gaDenID != null && loaiTauID != null && hangToaID != null) {
					...
					int gia = controller.getGiaForTooltip(chuyenID, gaDiID, gaDenID, loaiTauID, hangToaID);
	...
	private JButton createSeatButton(Ghe g, boolean isSelectedInSession, String tooltipPrice, boolean priceCalculated) {
		...
		b.addActionListener(e -> {
			...
			boolean wasSelectedNow = controller.getSelectedSoGhe(currentToa).contains(g.getSoGhe());
			...
			if (controller != null) {
				if (wasSelectedNow) {
					...
					controller.onSeatClicked(currentToa, g);
				}
			}
		});
	...
	private class LoadSeatWorker extends SwingWorker<List<Ghe>, Void> {
		...
		protected List<Ghe> doInBackground() throws Exception {
			...
			if (controller.getExchangeSession() != null) {
				SearchCriteria sc = controller.getExchangeSession().getCriteriaTimKiem();
				if (sc != null) {
					gaDiID = sc.getGaDiId();
