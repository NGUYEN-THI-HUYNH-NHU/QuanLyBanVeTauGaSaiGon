...
public class WizardController {
	...
	public void goToStep(int stepIndex, int tripIndex) {
		...
		if (stepIndex == 2) {
			...
			if (p == null) {
				...
				System.err.println(
						"WizardController.goToStep: PanelBuoc2 not registered for step 2 (cardName=" + cardName + ")");
				return;
			}
			...
			if (p instanceof PanelBuoc2) {
				...
				List<Chuyen> results = (tripIndex == 0) ? s.getOutboundResults() : s.getReturnResults();
				// call enter on UI panel on EDT (PanelBuoc2 should handle being called from
				// EDT)
				SwingUtilities.invokeLater(() -> {
					try {
				...
				System.err.println(
						"WizardController.goToStep: registered panel for step 2 is not an instance of PanelBuoc2. Actual class: "
								+ p.getClass().getName());
			}
		}
	}
	...
	public void goToStep(int stepIndex) {
		final String cardName = stepNameMap.get(stepIndex);
		if (cardName == null) {
			System.err.println("WizardController: Không tìm thấy cardName cho bước " + stepIndex);
			return;
		}
		SwingUtilities.invokeLater(() -> {
			layout.show(container, cardName);
