...
// (Giả sử bạn có các class này)
// import entity.PhieuGiuCho;
// import entity.ChiTietPhieuGiuCho;
...
public class BanVe1Controller {
    ...
    // === 1. Khai báo BUS xử lý nghiệp vụ ===
    ...
    private void initMediatorLogic() {
        ...
        this.buoc2Controller.addSeatSelectedListener(new SeatSelectedListener() {
			...
			public void onMuaVeClicked() {
                ...
                final List<VeSession> veTrongGio = bookingSession.getSelectedTicketsForTrip(
                    buoc2Controller.getCurrentTripIndex()
                );

                if (veTrongGio == null || veTrongGio.isEmpty()) {
                    JOptionPane.showMessageDialog(p1, "Giỏ vé trống. Vui lòng chọn ít nhất 1 vé.", "Lỗi", JOptionPane.WARNING_MESSAGE);
                    return;
                }
        ...
        new SwingWorker<Boolean, Void>() { 
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(p1, "Lỗi hệ thống: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        }.execute();
    }
    ...
    private void attachStep3Listeners() {
        ...
        p3.getConfirmButton().addActionListener(ev -> {
            ...
            for (PassengerRow r : rows) {
                VeSession v = r.getVeSession();
                // (Giả sử VeSession có hàm setHanhKhach(PassengerInfo))
                // v.setHanhKhach(r.getPassengerInfo()); 
            }
