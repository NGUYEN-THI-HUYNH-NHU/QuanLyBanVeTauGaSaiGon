...
public class PdfTicketExporter {
	...
	public void exportTicketsToPdf(BookingSession session) {
		...
		processDirectPrint(session.getAllSelectedTickets(), session.getDonDatCho());
	}
	...
	public void exportTicketsToPdf(Ve ve) {
		...
		processDirectPrint(List.of(new VeSession(ve)), ve.getDonDatCho());
	}
	...
	public void exportTicketsToPdf(ExchangeSession exchangeSession) {
		...
		processDirectPrint(exchangeSession.getListVeMoiDangChon(), exchangeSession.getDonDatChoMoi());
	}

	// 2. XỬ LÝ IN ẤN (RASTER MODE)
	private void processDirectPrint(List<VeSession> tickets, DonDatCho donDatCho) {
		try {
			File tempFile = File.createTempFile("Job_Print_", ".pdf");
			tempFile.deleteOnExit();
			createPdfDocument(tempFile, tickets, donDatCho);
	...
	private void createPdfDocument(File fileToSave, List<VeSession> tickets, DonDatCho donDatCho) {
		try (PDDocument document = new PDDocument()) {
			...
			for (VeSession ticket : tickets) {
				...
				try (PDPageContentStream cs = new PDPageContentStream(document, page)) {
					drawTicketK57(document, cs, ticket, donDatCho.getDonDatChoID());
				}
			}
	...
	private void drawTicketK57(PDDocument document, PDPageContentStream cs, VeSession ticket, String donDatChoID)
		...
		drawRightText(cs, fontBold, 9, ve.getGaDi().getTenGa(), PAGE_WIDTH - MARGIN_X, y);
		y -= 12;
		drawCenteredText(cs, fontRegular, 8, "v", centerX, y);
		y -= 12;
		drawLeftText(cs, fontRegular, 8, "Ga đến:", MARGIN_X, y);
		drawRightText(cs, fontBold, 9, ve.getGaDen().getTenGa(), PAGE_WIDTH - MARGIN_X, y);
		y -= 15;
		...
		// Chi tiết
		y = drawRowSmart(cs, "Tàu:", ve.getGhe().getToa().getTau().getTauID(), y);
		...
		cs.showText("TOA " + ve.getGhe().getToa().getSoToa());
		cs.endText();
		drawRightText(cs, fontBold, 10, "GHẾ " + ve.getGhe().getSoGhe(), PAGE_WIDTH - MARGIN_X, y);
		y -= 15;

		y = drawRowSmart(cs, "Hạng toa:", ve.getGhe().getToa().getHangToa().getDescription(), y);

		// Tên khách
		String tenKhach = ve.getKhachHang().getHoTen().toUpperCase();
		...
		y = drawRowSmart(cs, "Số giấy tờ:", ve.getKhachHang().getSoGiayTo(), y);
