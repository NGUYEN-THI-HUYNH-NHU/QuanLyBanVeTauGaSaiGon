...
public class BieuGiaVeTableModel extends AbstractTableModel {
    ...
    private List<BieuGiaVeDTO> rows;

    public BieuGiaVeTableModel() {
        this.rows = new ArrayList<>();
    }

    public void setRows(List<BieuGiaVeDTO> rows) {
        this.rows = rows;
        fireTableDataChanged();
    }

    public BieuGiaVeDTO getRow(int rowIndex) {
    ...
    public void addRow(BieuGiaVeDTO bg) {
        rows.add(bg);
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        BieuGiaVeDTO bg = rows.get(rowIndex);
        switch (columnIndex) {
            case COL_ID:
                return bg.getId();
            case COL_UU_TIEN:
