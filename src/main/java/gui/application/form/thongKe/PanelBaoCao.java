...
public class PanelBaoCao extends JPanel {
    ...
    public PanelBaoCao(String tenNV, String caLV, String ngayLV, double cashSystem, double transferSystem,
        ...
        if (tempUser == null) tempUser = new NhanVienDTO();
    ...
    private void initComponents() {
        ...
        pnlCenter.add(createTitlePanel());
        pnlCenter.add(createTablePanel());
        pnlCenter.add(Box.createVerticalStrut(10));
        pnlCenter.add(createSummaryAndButtonPanel());
        ...
        SwingWorker<ThongKeResult, Void> worker = new SwingWorker<>() {
            ...
            protected void done() {
                ...
                try {
                    ...
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(PanelBaoCao.this,
                            "Lỗi khi tải dữ liệu báo cáo: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            }
        };
        worker.execute();
            ...
            public void windowClosed(java.awt.event.WindowEvent windowEvent) {
                if (nhapTienMatDialog.isGiaoCaConfirmed()) {
                    giaoCaModel.setGiaoCaData(nhapTienMatDialog.getTienMatThucTeDaNhap(),
                            ...
                            nhapTienMatDialog.getChiTietTienMatDaNhap());
                    updateSummaryPanel();
                }
            }
        });
    ...
    private void updateTable(List<Object[]> list) {
        ...
        if (list != null) {
            ...
            for (Object[] row : list) {
                ...
                if (trangThai != null && trangThai.equals("Hoàn thành")) currentTotalSystem += tongTien;
            }
        }
    ...
    public void updateSummaryPanel() {
        ...
        if (lblTongValue != null) lblTongValue.setText(currencyFormatter.format(totalSystem).replace(",", "."));
    ...
    private void exportToExcel() {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            Sheet sheet1 = workbook.createSheet("ChiTietHoaDon");
            ...
            int startRowSheet1 = 4;
            Row header1 = sheet1.createRow(startRowSheet1);
            ...
            for (int row = 0; row < reportTableModel.getRowCount(); row++) {
                Row excelRow = sheet1.createRow(startRowSheet1 + 1 + row);
            ...
            for (int i = 0; i < reportTableModel.getColumnCount(); i++) sheet1.autoSizeColumn(i);

            // Sheet 2: Tổng kết
            Sheet sheet2 = workbook.createSheet("TongKet");
            int rowNum = 0;
            ...
            rowNum++;
            sheet2.createRow(rowNum++).createCell(0).setCellValue("CHI TIẾT TIỀN MẶT THỰC TẾ:");
            ...
            if (mapTien != null && !mapTien.isEmpty()) {
                ...
                for (Integer menhGia : sortedKeys) {
                    ...
                    Row rowTien = sheet2.createRow(rowNum++);
                    rowTien.createCell(0).setCellValue(currencyFormatter.format(menhGia));
                    rowTien.createCell(1).setCellValue("x " + soLuong);
                    rowTien.createCell(2).setCellValue("= " + currencyFormatter.format((double) menhGia * soLuong));
                }
            } else {
