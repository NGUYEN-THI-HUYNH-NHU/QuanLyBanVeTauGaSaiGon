...
public class GheDAO extends AbstractGenericDAO<Ghe, String> implements IGheDAO {
    ...
    public List<Ghe> getGheByGaDiGaDenChuyenToa(String gaDiID, String gaDenID, String chuyenID, String toaID) {
        return doInTransaction(em -> {
            ...
            for (Object[] row : results) {
                Ghe g = new Ghe();
                g.setGheID((String) row[0]); // gheID
                g.setSoGhe((Integer) row[1]); // soGhe

                Toa toa = new Toa();
                toa.setToaID((String) row[2]); // toaID
                g.setToa(toa);
    ...
    /**
     * 2. Tính toán giá vé (Native Query)
     */
