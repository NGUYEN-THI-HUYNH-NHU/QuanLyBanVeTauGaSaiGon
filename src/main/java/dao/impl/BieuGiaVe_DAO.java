    ...
    public List<BieuGiaVe> getAllBieuGia() {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                bg.setLoaiTauApDung(loaiTauID != null ? LoaiTau.builder().loaiTauID(loaiTauID).build() : null);

                String hangToaID = rs.getString("hangToaApDungID");
                bg.setHangToaApDung(hangToaID != null ? HangToa.builder().hangToaID(hangToaID).build() : null);
    ...
    public BieuGiaVe getBieuGiaByID(String id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                Date sqlDateBD = rs.getDate("ngayBatDau");
                LocalDate ngayBatDau = (sqlDateBD != null) ? sqlDateBD.toLocalDate() : null;

                Date sqlDateKT = rs.getDate("ngayKetThuc");
                ...
                if (loaiTauStr != null) {
                    try {
                        ...
                        for (LoaiTauEnums lt : LoaiTauEnums.values()) {
                            if (lt.name().equalsIgnoreCase(loaiTauStr)) {
                                loaiTau = new LoaiTau(loaiTauStr);
                                break;
                            }
                        }
                ...
                if (hangToaStr != null) {
                    try {
                        hangToa = new HangToa(hangToaStr);
                    } catch (IllegalArgumentException e) {
