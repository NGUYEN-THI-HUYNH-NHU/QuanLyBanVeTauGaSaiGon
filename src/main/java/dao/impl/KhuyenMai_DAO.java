...
public class KhuyenMai_DAO {
    ...
    public boolean themKhuyenMai(KhuyenMai km, DieuKienKhuyenMai dkkm) {
        ...
        try (Connection con = connectDB.getConnection()) {
            ...
            try (PreparedStatement psKM = con.prepareStatement(sqlKM);
                ...
                psKM.setDate(6, Date.valueOf(km.getNgayBatDau()));
                ...
                psDK.setString(4, dkkm.getLoaiTau() != null ? dkkm.getLoaiTau().getLoaiTauID() : null);
                psDK.setString(5, dkkm.getHangToa() != null ? dkkm.getHangToa().getHangToaID() : null);
                psDK.setString(6, dkkm.getLoaiDoiTuong() != null ? dkkm.getLoaiDoiTuong().getLoaiDoiTuongID() : null);
    ...
    public boolean suaKhuyenMai(KhuyenMai km, DieuKienKhuyenMai dk) {
        ...
        try {
            ...
            try (PreparedStatement psKM = con.prepareStatement(sqlUpdateKM)) {
                ...
                psKM.setDate(5, Date.valueOf(km.getNgayBatDau()));
            ...
            try (PreparedStatement psDK = con.prepareStatement(sqlUpdateDK)) {
                ...
                psDK.setString(2, dk.getLoaiTau() != null ? dk.getLoaiTau().getLoaiTauID() : null);
                psDK.setString(3, dk.getHangToa() != null ? dk.getHangToa().getHangToaID() : null);
                psDK.setString(4, dk.getLoaiDoiTuong() != null ? dk.getLoaiDoiTuong().getLoaiDoiTuongID() : null);
    ...
    public List<KhuyenMai> timKhuyenMai(String tuKhoa, String maTuyen, Boolean trangThai,
                                        ...
                                        LoaiTau loaiTau, HangToa hangToa, LoaiDoiTuong loaiDoiTuongEnums) {
        ...
        if (ngayBD != null) {
            ...
            params.add(Date.valueOf(ngayBD));
        }
        if (ngayKT != null) {
            sql.append(" AND km.ngayKetThuc <= ? ");
            params.add(Date.valueOf(ngayKT));
        }

        // Loại tàu
        if (loaiTau != null) {
            sql.append(" AND dk.loaiTauID = ? ");
            params.add(loaiTau.getLoaiTauID());
        }

        // Hạng toa
        if (hangToa != null) {
            sql.append(" AND dk.hangToaID = ? ");
            params.add(hangToa.getHangToaID());
        }

        // Loại đối tượng
        if (loaiDoiTuongEnums != null) {
            sql.append(" AND dk.loaiDoiTuongID = ? ");
            params.add(loaiDoiTuongEnums.getLoaiDoiTuongID());
        }
    ...
    public DieuKienKhuyenMai layDieuKienKhuyenMaiTheoKhuyenMai(String khuyenMaiID) {
        ...
        try (Connection con = connectDB.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                if (loaiTauStr != null) {
                    try {
                        dk.setLoaiTau(new LoaiTau(loaiTauStr));
                ...
                if (hangToaStr != null) {
                    try {
                        dk.setHangToa(new HangToa(hangToaStr));
                    } catch (IllegalArgumentException e) {
                ...
                if (loaiDTStr != null) {
                    try {
                        dk.setLoaiDoiTuong(new LoaiDoiTuong(loaiDTStr));
                    } catch (IllegalArgumentException e) {
