...
public class PhieuDungPhongVIPDAO extends AbstractGenericDAO<PhieuDungPhongVIP, String> implements IPhieuDungPhongVIPDAO {
    ...
    @Override
    public boolean insertPhieuDungPhongVIP(PhieuDungPhongVIP phieuDungPhongChoVIP) {
        return doInTransaction(em -> {
            String sql = "INSERT INTO PhieuDungPhongVIP (phieuDungPhongVIPID, dichVuPhongChoVIPID, veID, trangThai) "
                    + "VALUES (?1, ?2, ?3, ?4)";
            Query query = em.createNativeQuery(sql);
            query.setParameter(1, phieuDungPhongChoVIP.getPhieuDungPhongVIPID());
            query.setParameter(2, phieuDungPhongChoVIP.getDichVuPhongChoVIP().getDichVuPhongChoVIPID());
            query.setParameter(3, phieuDungPhongChoVIP.getVe().getVeID());
            query.setParameter(4, phieuDungPhongChoVIP.getTrangThai().toString());
    ...
    @Override
    public PhieuDungPhongVIP getPhieuDungPhongVIPByID(String phieuDungPhongChoVIPID) {
        return doInTransaction(em -> {
            String sql = "SELECT phieuDungPhongVIPID, dichVuPhongChoVIPID, veID, trangThai "
                    + "FROM PhieuDungPhongVIP WHERE phieuDungPhongVIPID = ?1";
            Query query = em.createNativeQuery(sql);
            query.setParameter(1, phieuDungPhongChoVIPID);
