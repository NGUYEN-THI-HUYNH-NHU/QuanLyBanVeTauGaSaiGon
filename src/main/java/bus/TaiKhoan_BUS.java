...
public class TaiKhoan_BUS {
    ...
    public boolean themTaiKhoan(TaiKhoan tk) {
        ...
        if (ok) {
            ghiLog(tk.getId(), nhanVienHienTai != null ? nhanVienHienTai.getId() : null,
                    entity.type.NhatKyAudit.THEM, "Thêm tài khoản: " + "Mã tài khoản " + tk.getId() + "\n"
    ...
    public boolean suaTaiKhoan(TaiKhoan tkMoi) {
        ...
        // 1. Lay tai khoan cu
        TaiKhoan tkCu = taiKhoan_dao.timTaiKhoanTheoID(tkMoi.getId());
        if (tkCu == null) {
            return false;
        }
        ...
        if (ok) {
            ghiLog(tkMoi.getId(), nhanVienHienTai != null ? nhanVienHienTai.getId() : null,
