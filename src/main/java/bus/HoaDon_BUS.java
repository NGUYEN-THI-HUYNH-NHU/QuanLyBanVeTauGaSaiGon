...
public class HoaDon_BUS {
    ...
    public HoaDon taoHoaDonHoanVe(DonDatCho donDatCho, KhachHang khachHang, NhanVien nhanVien, double tongTienHoan) {
        ...
        hoaDon.setId("HDHV-" + donDatCho.getId().substring(4));
    ...
    public List<HoaDonChiTiet> taoCacHoaDonChiTietBanVe(HoaDon hoaDon, List<VeSession> listVeMoi) {
        ...
        for (VeSession ve : listVeMoi) {
            String hdctVeID = hoaDon.getId() + "-" + (++stt);
            ...
            if (ve.getPhieuDungPhongVIP() != null) {
                String hdctPhieuID = hoaDon.getId() + "-" + (++stt);
            ...
            if (ve.getVe().getKhachHang().getLoaiDoiTuong().getId().equals(LoaiDoiTuong.TRE_EM.toString())) {
                String hdctGiamDTID = hoaDon.getId() + "-" + (++stt);
            ...
            if (ve.getKhuyenMaiApDung() != null && ve.getKhuyenMaiApDung().getId() != null) {
                String hdctKMID = hoaDon.getId() + "-" + (++stt);
    ...
    public List<HoaDonChiTiet> taoCacHoaDonChiTietHoanVe(Connection conn, HoaDon hoaDon,
        ...
        for (VeHoanRow row : listVeHoanRow) {
            ...
            PhieuDungPhongVIP phieu = phieuDungPhongVIPDAO.getPhieuDungPhongVIPByVeID(conn, row.getVe().getId());
            if (phieu != null) {
                String hdctPhieuID = hoaDon.getId() + "-" + (++stt);
            ...
            String hdctLePhiID = hoaDon.getId() + "-" + (++stt);
    ...
    public List<HoaDonChiTiet> taoCacHoaDonChiTietDoiVe(Connection conn, HoaDon hoaDon,
        ...
        for (int i = 0; i < soLuongVe; i++) {
            // Dòng vé đổi
            String hdctVeDoiID = hoaDon.getId() + "-" + (++stt);
            ...
            // Dòng vé mới
            String hdctVeMoiID = hoaDon.getId() + "-" + (++stt);
            ...
            if (phieuMoi != null) {
                if (phieuDungPhongVIPDAO.getPhieuDungPhongVIPByID(conn, phieuMoi.getId()) == null) {
                    String hdctPhieuID = hoaDon.getId() + "-" + (++stt);
            ...
            if (listVeMoi.get(i).getVe().getKhachHang().getLoaiDoiTuong().getId().equals(LoaiDoiTuong.TRE_EM)) {
                String hdctGiamDTID = hoaDon.getId() + "-" + (++stt);
