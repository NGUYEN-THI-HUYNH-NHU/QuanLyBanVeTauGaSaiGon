...
public class Dashboard_DAO {
	...
	/**
	 * Tính tổng doanh thu. Đã xóa "trangThai = 1". Thêm điều kiện loại trừ hóa đơn
	 * hoàn/đổi (HDHV, HDDV) để ra doanh thu thực bán.
	 */
	...
	private Map<LocalDate, Integer> getInvoiceData(LocalDate s, LocalDate e, String datePart, boolean isRefund) {
		...
		String cond = isRefund ? "(hoaDonID LIKE 'HDHV%' OR hoaDonID LIKE 'HDDV%')"
				: "(hoaDonID NOT LIKE 'HDHV%' AND hoaDonID NOT LIKE 'HDDV%')";

		StringBuilder sql = new StringBuilder(
				"SELECT " + datePart + " AS Ngay, COUNT(hoaDonID) AS SoLuong FROM HoaDon WHERE " + cond);
		if (s != null) {
			sql.append(" AND thoiDiemTao >= ?");
		}
		if (e != null) {
	...
	public Map<LocalDate, Map<String, Integer>> getTicketsBySeatTypeOverTime(LocalDate startDate, LocalDate endDate) {
		// Query này join qua HoaDon nhưng điều kiện quan trọng là Ve.trangThai
		// ('DA_BAN') nên không ảnh hưởng nhiều
